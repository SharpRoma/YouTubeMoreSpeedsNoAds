...
// @version      1.1.0
// @description  Removes ads, shorts, and adds more playback speed buttons.
// @author       GoodTube Embed, SharpRoma
// @match        *://m.youtube.com/*
...
(function () {
    ...
    // START MoreSpeeds Integration: Helper function from original script
    ...
    // START MoreSpeeds Integration: Logic for adding speed buttons
    ...
    function addMoreSpeedsButtons() {
        ...
        // If title exists but buttons already exist, do nothing.
        if (titleElem && document.getElementById('more-speeds')) {
            return;
        }
        // If title doesn't exist, try again later.
        if (!titleElem) {
            return;
        }
        ...
        moreSpeedsDiv.style.marginTop = '8px'; // Add some space
        ...
        for (let i = 1; i < 4.25; i += 0.25) {
            ...
            btn.addEventListener('click', () => {
                // MODIFIED: Send message to iframe instead of direct manipulation
                if (goodTube_player && goodTube_player.contentWindow) {
                    goodTube_player.contentWindow.postMessage('goodTube_setSpeed_' + i, '*');
                }
        ...
        restoreSpeed(); // Restore speed immediately
    ...
    function restoreSpeed() {
        ...
        if (savedSpeed) {
            // MODIFIED: Send message to iframe
            if (goodTube_player && goodTube_player.contentWindow) {
    ...
    function removeMoreSpeedsButtons() {
        ...
        if (moreSpeedsInterval) {
            clearInterval(moreSpeedsInterval);
            moreSpeedsInterval = null;
        }
    }
    // END MoreSpeeds Integration
	...
	function goodTube_actions() {
		...
		if (previousUrl !== currentUrl) {
			...
			if (window.location.href.indexOf('.com/watch') !== -1) {
                ...
                // START MoreSpeeds Integration: Add buttons on video page
			...
			// Otherwise if we're not viewing a video
				...
				// Clear the player
				goodTube_player_clear();
                // START MoreSpeeds Integration: Remove buttons when not on a video page
	...
	function goodTube_iframe_receiveMessage(event) {
        ...
        // START MoreSpeeds Integration: Listen for speed changes
        else if (event.data.indexOf('goodTube_setSpeed_') !== -1) {
            const speed = parseFloat(event.data.replace('goodTube_setSpeed_', ''));
            const videoElement = document.querySelector('video');
            if (videoElement && !isNaN(speed)) {
